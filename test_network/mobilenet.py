...
def get_symbol(num_classes, **kwargs):
    ...
    fc7 = mx.symbol.Flatten(data=pool6, name='unfronzen_fc_flatten')
    fc7 = mx.symbol.FullyConnected(data=fc7, num_hidden=512, name="unfronzen_fc7")
    slice_fc7 = mx.symbol.SliceChannel(data=fc7, num_outputs=2, name="slice_fc7")
    feature = mx.symbol.maximum(slice_fc7[0], slice_fc7[1])
    drop1 = mx.symbol.Dropout(data=feature, p=0.5, name="drop1")
    fc8 = mx.symbol.FullyConnected(data=drop1, num_hidden=num_classes, name="unfronzen_fc8")

    softmax = mx.symbol.SoftmaxOutput(data=fc8, name='softmax')

    # return softmax
    l2_feature = mx.symbol.L2Normalization(name='facefeature_norm', data=feature)
    return mx.sym.Group([softmax, mx.sym.BlockGrad(l2_feature, name='facefeature')])

if __name__ == '__main__':
    network = get_symbol(1000)
    a = mx.viz.plot_network(network, shape={'data':(1, 3, 160, 160)})
    a.view()